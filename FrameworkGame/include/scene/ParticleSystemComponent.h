 ...
 *        shader.
 *
 * Opaque to the simulation: nothing here is read or written by the emitter, it
 * is only copied through to the GPU. See @ref gfx::QuadCustomData for why it is
...
class ParticleSystemComponent : public ComponentT<ParticleSystemComponent> {
 public:
  ...
  /**
   * @brief Spawn @p count particles immediately from the current config or the
   *        @p overrideConfig if present.
   */
  void
  emit(size_t count, const std::optional<EmitterConfig>& overrideConfig = std::nullopt);
  ...
  /**
   * @brief Returns the normalised progress of this
   *        emitter (0 = just started, 1 = finished).
   */
  NODISCARD float
  getProgress() const;
   ...
   *        Non-owning; the material component outlives the draw.
   *        Null clears it.
   */
  FORCEINLINE void
   ...
   */
  NODISCARD FORCEINLINE MaterialComponent*
  ...
  /**
   * @brief Head of the active list, for a read-only walk via @c
   *        Particle::next. The emitter owns the list; the pointer dies with the
   *        next update.
   */
  ...
  /**
   * @brief Serializes the emitter config (texture by asset UUID), sort mode
   *        and world-space flag. Live particles and timers are transient and
   *        not saved.
   */
  void
  ...
  /**
   * @brief Restores the emitter config (re-resolving the texture via
   *        AssetManager) and flags via @ref setConfig; no live
   *        particles are restored.
   */
  void
 ...
 private:
  ...
  /**
   * @brief Allocate a new particle, initialised from the current
   *        config or @p overrideConfig if present.
   */
  void
  spawnParticle(const std::optional<EmitterConfig>& overrideConfig = std::nullopt);
   ...
   */
  void
  rebuildVertices() const;

  /**
   * @brief Fill the buffer with one instance per particle (instanced path).
   *
   * Streams 20 bytes per particle instead of the legacy path's six vertices,
   * and leaves the corner rotation and size interpolation to the vertex shader.
   */
  void
  rebuildInstances() const;

  /**
   * @brief Choose between the instanced and legacy draw paths, once.
   *
   * Deferred to the first draw: the buffer's element count differs per path,
   * and the decision depends on state that only exists once there is a live
   * context.
   */
  void
  ...
  /** @brief Keep-alive for an asset-backed texture; @ref m_config.texture
   * points into it. */
  ...
  /** @brief Accumulated time since the last auto-spawn (fractional emission
   * rates). */
   ...
   *         legacy one. Lazily created at first draw to avoid an OpenGL context
   *         dependency. */
  mutable UniquePtr<sf::VertexBuffer> m_vertexBuffer;
  /** @brief Set to true whenever the active list changes, triggers a rebuild
   * before draw. */
  mutable bool m_verticesDirty = true;
  /** @brief True once @ref resolveDrawPath has picked a path for this
   * component. */
  mutable bool m_drawPathResolved = false;
  /** @brief True while drawing through the instanced path; false = legacy
   * vertices. */
