...
UIWidget::~UIWidget()
{
    // Widget is owned by Canvas via unique_ptr — destruction is handled
    // automatically when the Canvas dies. Do NOT call destroyWidget here:
    // that would modify the canvas widget vector while the vector itself
    // is being destroyed (during ~Canvas), causing a double-free.
    // m_canvas is intentionally not cleared so the Canvas can still be
    // reached during widget destruction if needed.
}
