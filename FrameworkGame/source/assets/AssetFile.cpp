...
#include "lz4hc.h"                 // vendored LZ4 HC (ThirdParty/lz4)
...
// LZ4 HC compression level (2-12). Cooking is a one-time offline step, so we
// use the highest HC level for the best ratio. LZ4 decompression speed is
// roughly constant across levels, so a high cook level costs nothing on load.
constexpr int kLz4Level = 12;
...
compressLz4(const uint8* src, size_t srcSize, Vector<uint8>& out) {
  // LZ4_compressBound gives the worst-case upper bound for LZ4_compress_default
  // (which also bounds HC), so a single resize() reserves enough up front.
  const int srcSizeI = static_cast<int>(srcSize);
  const int bound = LZ4_compressBound(srcSizeI);
  out.resize(static_cast<size_t>(bound));
  const int written = LZ4_compress_HC(
      reinterpret_cast<const char*>(src),
      reinterpret_cast<char*>(out.data()),
      srcSizeI, bound, kLz4Level);
  if (written < 0) {
    out.clear();
    return false;
  }
...
AssetFileWriter::addChunk(const void* data,
  ...
  // Default to uncompressed; only flip to kLZ4 below if compression actually
...
AssetFileReader::readChunk(size_t index, Vector<uint8>& out) const {
  ...
  if (ChunkCompression::kLZ4 != entry.compression) {
    return false;  // unsupported codec: fail cleanly, never the loop
  }
